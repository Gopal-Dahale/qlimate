...
def _iao_occupied_orbitals(mol, mf):
    ...
    c_tilde = np.dot(c_tilde,
                     orth.lowdin(reduce(np.dot, (c_tilde.T, s1, c_tilde))))
    ...
    iao_active = (p12 + reduce(np.dot, (ccs1, ctcts1, p12)) * 2 -
                  np.dot(ccs1, p12) - np.dot(ctcts1, p12))

    #   Orthogonalize A
    iao_active = np.dot(
        iao_active, orth.lowdin(reduce(np.dot,
                                       (iao_active.T, s1, iao_active))))
...
def _iao_complementary_orbitals(mol, iao_ref):
    ...
    #   Calculate A = ccs1 * ctcts1 * p12 + ( 1 - ccs1 ) * ( 1 - ctcts1 ) * p12
    iao_comp = (p12 + reduce(np.dot, (ccs1, ctcts1, p12)) * 2 -
                np.dot(ccs1, p12) - np.dot(ctcts1, p12))
    iao_comp = np.dot(iao_comp,
                      orth.lowdin(reduce(np.dot, (iao_comp.T, s1, iao_comp))))
...
def _iao_atoms(mol, iao1, iao2):
    ...
    iao_combine = np.dot(
        iao_combine,
        orth.lowdin(reduce(np.dot, (iao_combine.T, s1, iao_combine))))
