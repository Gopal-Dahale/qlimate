...
def dmet_fragment_guess(t_list, bath_orb, chemical_potential, norb_high,
                        number_active_electron, active_fock):
    ...
    fock_fragment = reduce(
        np.dot,
        (bath_orb[:, :norb_high].T, active_fock, bath_orb[:, :norb_high]))
    norb = t_list[0]
    ...
    frag_guess = np.dot(
        eigenvectors[:, :int(number_active_electron / 2)],
        eigenvectors[:, :int(number_active_electron / 2)].T) * 2
