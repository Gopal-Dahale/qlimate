...
class VQESolver:
    ...
    def simulate(self, molecule, mean_field=None, **kwargs):
        ...
        occupied = kwargs.get('occupied', None)
        active = kwargs.get('active', None)
        self.hardware_backend = self.hardware_backend_type(
            self.ansatz_type, molecule, mean_field, occupied, active)
        ...
        if self.initial_amplitudes is not None:
            ...
            amplitudes = np.random.uniform(
                size=self.hardware_backend.amplitude_dimension)
            amplitudes = np.array(amplitudes, requires_grad=True)
        if self.verbose:
