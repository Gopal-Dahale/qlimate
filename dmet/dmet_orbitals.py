...
class dmet_orbitals:
    ...
    def __init__(self, mol, mf, active_space, localization_function):
        ...
        low_scf_dm = reduce(np.dot,
                            (mf.mo_coeff, np.diag(mf.mo_occ), mf.mo_coeff.T))

        
        start = time.time()
        low_scf_twoint = scf.hf.get_veff(mf.mol, low_scf_dm, 0, 0, 1)
        print("Time for two-electron integrals(low_scf_twoint):",
              time.time() - start)

        self.low_scf_fock = mf.mol.intor('cint1e_kin_sph') + mf.mol.intor(
            'cint1e_nuc_sph') + low_scf_twoint

        # Define the active space if possible
        self.dmet_active_orbitals = np.zeros([mf.mol.nao_nr()], dtype=int)
        print("Dmet active orbitals:", self.dmet_active_orbitals)

        self.dmet_active_orbitals[active_space] = 1
        print("Dmet active orbitals:", self.dmet_active_orbitals)

        self.number_active_orbitals = np.sum(self.dmet_active_orbitals)
        print("Number of active orbitals:", self.number_active_orbitals)

        print("Number of electrons:", mf.mol.nelectron)
        print("orbital occupancy:", mf.mo_occ, len(mf.mo_occ))
        print("orbital occupancy:", mf.mo_occ[self.dmet_active_orbitals == 0])

        self.number_active_electrons = int(
            np.rint(mf.mol.nelectron -
                    np.sum(mf.mo_occ[self.dmet_active_orbitals == 0])))

        print("Number of active electrons:", self.number_active_electrons)
        ...
        core_ao_dm = reduce(np.dot,
                            (mf.mo_coeff, np.diag(core_mo_dm), mf.mo_coeff.T))

        start = time.time()
        core_twoint = scf.hf.get_veff(mf.mol, core_ao_dm, 0, 0, 1)
        print("Time for two-electron integrals(core_twoint):",
              time.time() - start)
        ...
        self.core_constant_energy = mf.mol.energy_nuc() + np.einsum(
            'ij,ij->', core_oneint - 0.5 * core_twoint, core_ao_dm)
        self.active_oneint = reduce(
            np.dot, (self.localized_mo.T, core_oneint, self.localized_mo))
        self.active_fock = reduce(
            np.dot,
            (self.localized_mo.T, self.low_scf_fock, self.localized_mo))
    ...
    def dmet_fragment_hamiltonian(self, bath_orb, norb_high, onerdm_core):
        ...
        frag_oneint = reduce(np.dot,
                             (bath_orb[:, :norb_high].T, self.active_oneint,
                              bath_orb[:, :norb_high]))

        # Calculate the fock matrix
        density_matrix = reduce(
            np.dot, (self.localized_mo, onerdm_core, self.localized_mo.T))

        start = time.time()
        two_int = scf.hf.get_veff(self.mol_full, density_matrix, 0, 0, 1)
        print("Time for two-electron integrals(two_int):", time.time() - start)

        new_fock = self.active_oneint + reduce(np.dot, (
            (self.localized_mo.T, two_int, self.localized_mo)))
        frag_fock = reduce(
            np.dot,
            (bath_orb[:, :norb_high].T, new_fock, bath_orb[:, :norb_high]))
        ...
        print("Time for frag_twoint:", time.time() - start)
