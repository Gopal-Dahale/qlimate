...
def dmet_fragment_bath(mol, t_list, temp_list, onerdm_low):
    ...
    print("Time for dmet_onerdm_embed: ", time.time() - start)
    ...
    print("Time for dmet_bath_orb_sort: ", time.time() - start)

    # Add the core contribution

    start = time.time()
    bath_orb, e_core = dmet_add_to_bath_orb(mol, t_list, temp_list, e_sorted,
                                            c_sorted)
    print("Time for dmet_add_to_bath_orb: ", time.time() - start)
...
def dmet_bath_orb_sort(t_list, e_before, c_before):
    ...
    thresh_orb = np.sum(
        -np.maximum(-e_before, e_before - 2.0)[new_index] > 1e-13)
...
def dmet_add_to_bath_orb(mol, t_list, temp_list, e_before, c_before):
    ...
    norbital_total = mol.nao_nr()
    e_occupied_core_orbitals = np.hstack(
        (np.zeros([t_list[0] + t_list[1]]), add_e))
